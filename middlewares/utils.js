...
class Utils {
  ...
  async getInfoViaNIN(params, nin) {
    return fetch(
          ...
          String(nin),
      {
          ...
          headers: {
              ...
              Authorization: "Bearer " + process.env.VERIFY_ME_LIVE,
          },
      }
  )
  }
  
  async getInfoViaBVN(body) {
    return fetch("https://api.paystack.co/bvn/match", {
      method: "post",
      body: JSON.stringify(body),
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + process.env.PAYSTACKS_LIVE_SECRET,
      },
    })
  }
}
